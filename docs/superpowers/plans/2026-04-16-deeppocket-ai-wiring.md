...
**Source root:** `DeepPocket.ios/` (relative to the working tree)
...
**New files (5):**
- `DeepPocket.ios/Services/FoundationModelClient.swift` — on-device LLM wrapper (availability, structured output, chunking, cache, cancellation, logging).
- `DeepPocket.ios/Services/CalendarImportService.swift` — `EKEventStore` wrapper for import-only agenda.
- `DeepPocket.ios/Intents/DeepPocketIntents.swift` — four `AppIntent` types.
- `DeepPocket.ios/Views/MeetingShareSheet.swift` — `UIActivityViewController` wrapper.
- `DeepPocket.ios/AppRouter.swift` — `@Observable` coordinator for intent → app navigation.

**Modified files (~9):**
- `DeepPocket.ios/Services/LocalInsightGenerator.swift` — fill in FM stub, add concurrency guard.
- `DeepPocket.ios/Services/MeetingAssistantEngine.swift` — add FM-backed async `answer(...)`.
- `DeepPocket.ios/Services/MeetingExportService.swift` — add `shareItems(for:)` and `exportAll()`.
- `DeepPocket.ios/Services/DeepPocketShortcuts.swift` — register four intents.
- `DeepPocket.ios/DeepPocket_iosApp.swift` — own `AppRouter`, inject via `.environment`, call `updateAppShortcutParameters`.
- `DeepPocket.ios/ContentView.swift` — react to `AppRouter` requests; pass event-context to `RecordingRoomView`.
- `DeepPocket.ios/Views/DashboardView.swift` — Today's agenda + Upcoming + denied-state card.
- `DeepPocket.ios/Views/SettingsView.swift` — "Export all meetings" button.
- `DeepPocket.ios/Views/InsightView.swift` — per-meeting share button.
- `DeepPocket.ios/Views/RecordingRoomView.swift` — optional `initialContext` init param for calendar tap-to-record.

**Deleted files (1):**
- `DeepPocket.ios/Services/CalendarIntegrationService.swift` — export-back is out of scope; `CalendarImportService` replaces it with a different API shape.
...
Task 15 (DeepPocketShortcuts + App entry wiring) ← 2, 14        │
...
- [ ] **Step 1: Build & launch on iOS Simulator (iPhone 15, non-Pro — no Apple Intelligence).**
  ...
  Run in Xcode: `⌘R` with target scheme `DeepPocket.ios`, destination `iPhone 15 Simulator (iOS 18.2)`.
...
**Files:**
- Create: `DeepPocket.ios/AppRouter.swift`
- Create: `DeepPocket.ios/DeepPocketLog.swift`

- [ ] **Step 1: Create `DeepPocketLog.swift`.**
...
- [ ] **Step 3: Verify it compiles.**
  ...
  In Xcode: ⌘B.
  Expected: Build Succeeded. `AppRouter` and `DeepPocketLog` are not used yet — that's fine.
...
**Files:**
- Create: `DeepPocket.ios/Services/FoundationModelClient.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Services/FoundationModelClient.swift`
...
- [ ] **Step 5: Purge cache on app background.**
  ...
  Add a public method that `DeepPocket_iosApp` will call in Task 15:
...
**Files:**
- Modify: `DeepPocket.ios/Services/LocalInsightGenerator.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Services/MeetingAssistantEngine.swift`
- Modify: `DeepPocket.ios/Views/AssistantView.swift` (call site update)
...
- [ ] **Step 3: Update `AssistantView` to call the async path.**
  ...
  Open `DeepPocket.ios/Views/AssistantView.swift`. Find where `engine.answer(...)` is called and update the call site to `async/await`. If `AssistantView` has a submit handler like:
...
**Files:**
- Delete: `DeepPocket.ios/Services/CalendarIntegrationService.swift`
- Create: `DeepPocket.ios/Services/CalendarImportService.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Views/RecordingRoomView.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Views/DashboardView.swift`
- Modify: `DeepPocket.ios/ContentView.swift` (to carry `initialContext` through the sheet)
...
**Files:**
- Modify: `DeepPocket.ios/Services/MeetingExportService.swift`
...
**Files:**
- Create: `DeepPocket.ios/Views/MeetingShareSheet.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Views/InsightView.swift`
...
**Files:**
- Modify: `DeepPocket.ios/Views/SettingsView.swift`
...
**Files:**
- Create: `DeepPocket.ios/Intents/DeepPocketIntents.swift`
...
- [ ] **Step 1: Create the file with the four intents.**
  ...
  struct TodaysAgendaIntent: AppIntent {
      ...
      func perform() async throws -> some IntentResult & ProvidesDialog {
          ...
          DeepPocketLog.intents.debug("TodaysAgendaIntent: \(events.count) events")
  ...
  struct AskDeepPocketIntent: AppIntent {
      ...
      func perform() async throws -> some IntentResult & ProvidesDialog {
          ...
          DeepPocketLog.intents.debug("AskDeepPocketIntent: usedFM=\(result.usedFM), cites=\(result.citedMeetings.count)")
...
## Task 15: DeepPocketShortcuts + App entry wiring
...
**Files:**
- Modify: `DeepPocket.ios/Services/DeepPocketShortcuts.swift`
- Modify: `DeepPocket.ios/DeepPocket_iosApp.swift`
- Modify: `DeepPocket.ios/ContentView.swift`

- [ ] **Step 1: Register the four intents in `DeepPocketShortcuts`.**

  Open `DeepPocket.ios/Services/DeepPocketShortcuts.swift`. Replace the contents with:
...
- [ ] **Step 2: Own the `AppRouter` in `DeepPocket_iosApp`.**

  Open `DeepPocket.ios/DeepPocket_iosApp.swift`. Add imports + state + inject:
  ...
  Also note: the existing body of `DeepPocket_iosApp` may differ — keep any existing modifiers (theme, analytics, etc.) that are already present; the additions are `@State private var router`, the `init`, `.environment(router)`, and the scenePhase observer.
...
- [ ] **Step 6: Checkpoint.**
  ...
  Zero warnings; smoke tests pass. If a shortcut doesn't appear in Settings, confirm the `DeepPocketShortcuts` type is the **only** `AppShortcutsProvider` in the target — AppIntents only allows one.
...
- [ ] **Step 3: Long transcript.**
  ...
  - Cancellation: navigate away mid-generation → log shows "FM insight generation cancelled" (via Console.app filter on subsystem `works.kaspar.deeppocket`, category `ai`).
