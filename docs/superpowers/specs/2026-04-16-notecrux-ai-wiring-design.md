...
Make every advertised feature of NoteCrux actually work end-to-end. Two parts:
...
- **Wire the three orphaned services** — `CalendarIntegrationService`, `NoteCruxShortcuts`, `MeetingExportService` — into reachable UI surfaces.
...
| `NoteCruxShortcuts` | Registered in `NoteCruxApp.init()`; backed by 4 new `AppIntent` types | Surfaces in Spotlight, Siri, Shortcuts app |
...
- `NoteCrux/Services/FoundationModelClient.swift` — single shared `LanguageModelSession` wrapper, availability check, chunking helper, `@Generable` structured-output types, error handling.
- `NoteCrux/Services/CalendarImportService.swift` — wraps `EKEventStore`, exposes `upcomingEvents() async -> [CalendarEventSummary]`. Replaces `CalendarIntegrationService.swift`.
- `NoteCrux/Intents/DeepPocketIntents.swift` — `StartRecordingIntent`, `TodaysAgendaIntent`, `AskDeepPocketIntent`, `LastMeetingDecisionsIntent`. Existing `NoteCruxShortcuts` `AppShortcutsProvider` declaration updated to register them.
- `NoteCrux/Views/MeetingShareSheet.swift` — `UIViewControllerRepresentable` wrapping `UIActivityViewController`.

### 4.2 Modified files

- `NoteCrux/Services/LocalInsightGenerator.swift` — replace stub body at `:241-244` with real `FoundationModelClient.generateInsights(from:)` call inside a do/catch that falls back to `generateHeuristicInsights` on any throw.
- `NoteCrux/Services/MeetingAssistantEngine.swift` — add FM-backed answer path; existing keyword-search becomes the fallback when FM is unavailable or throws.
- `NoteCrux/NoteCruxApp.swift` — call `NoteCruxShortcuts.updateAppShortcutParameters()` in `init`.
- `NoteCrux/Views/DashboardView.swift` — "Today's agenda" section driven by `CalendarImportService`; tap-to-record-with-event-context.
- `NoteCrux/Views/SettingsView.swift` — "Calendar access" toggle + "Export all meetings" button.
- `NoteCrux/Views/InsightView.swift` (or current per-meeting detail view) — share button presenting `MeetingShareSheet`.
- `NoteCrux/Services/MeetingExportService.swift` — add public `shareItems(for: Meeting) -> [Any]` returning markdown + audio file URL.
...
- `StartRecordingIntent` — foreground, opens app, navigates to Dashboard, starts recording. Implemented via app launch handoff into a shared navigation/recording coordinator (a small `AppRouter` observable owned by `NoteCruxApp` that intents post a "start recording" request to and that `ContentView` / `DashboardView` react to).
